  ...
  return (
     ...
     <div className='mt-6'>
      <form onSubmit={(e)=>{
        ...
        className='bg-[#eeeeee] px-6 py-4 text-base font-mono rounded-lg w-full mt-4' type="text" placeholder='Enter OTP'/>
          <Link to={"/captain-riding"} onClick={()=>{
           props.setConfirmRidePopUpPanel(true)
         }} className='w-full bg-green-600 flex items-center justify-center text-lg text-white font-semibold p-2 mt-5 rounded-lg'>Confirm</Link>
         <button onClick={()=>{
           props.setConfirmRidePopUpPanel(false)
           props.setRidePopUpPanel(false)
         }} className='w-full bg-red-600 text-white font-semibold p-2 mt-2 text-lg rounded-lg'>Cancel </button>
      </form>
     </div>
    </div>
  )
